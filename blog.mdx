...
Creating a web show recommender for Netflix involves scraping data from web pages efficiently. In this article, we'll guide you through the process of using Crawlee. By the end of this guide, you'll have a functional web show recommender that can fetch and scrape Netflix data to provide show suggestions. 

Whether you're a developer looking to enhance your scraping skills or a Netflix enthusiast wanting to build a web-show recommendation engine, this project has something valuable for you. Let's dive in and explore how Crawlee can help us achieve this.
...
:::note
`cheerio` is not bundled with Crawlee to reduce install size and allow greater flexibility. Therefore we need to install both libraries separately.
:::
...
Additionally, Crawlee comes with the support of [Playwright](https://playwright.dev/) and [Puppeteer](https://pptr.dev/) for websites that are JavaScript-heavy in nature. 
...
In this project, we used Crawlee to scrape Netflix; similarly, Crawlee can be used to scrape Single Application Pages (SPAs) to JavaScript-heavy websites. The best part is all of this can be done while coding in JavaScript/TypeScript and using a single library.

If you want to learn more about Crawlee, I recommend going through their [documentation](https://crawlee.dev/docs/quick-start) and the [Crawlee web scraping tutorial by Apify](https://blog.apify.com/crawlee-web-scraping-tutorial/).
