...
Scraping data from websites Netflix is not as easy as scraping data from any regular website. Netflix website heavily [depends on JavaScript](https://medium.com/dev-channel/a-netflix-web-performance-case-study-c0bcde26a9d9) and it consists tons of elements. Crawlee has the capability of scraping JS-heavy websites using its headless browsers, which we will discuss further

[Crawlee](https://crawlee.dev/) helps us crawl websites, scrape data from them, and store the scraped data at the defined location.

Crawlee's features—autoscaling, persistent URL queue, headless browser support —make website scraping super easy.
...
Firstly, we will create a React app (for frontend) using Vite. You can check out the [ReactJS Docs](https://react.dev/) to create a React app.
...
`playwright` is not bundled with Crawlee to reduce install size and allow greater flexibility. Therefore we need to install both libraries separately.
